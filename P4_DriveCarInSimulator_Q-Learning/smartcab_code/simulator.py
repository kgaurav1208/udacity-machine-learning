            ...
            while True:
                ...
                try:
                    ...
                    if self.quit or self.env.done:
                        break
            temp = float((self.env.primary_agent.invalidMoves))/(self.env.primary_agent.invalidMoves + self.env.primary_agent.validMoves)
            if self.env.primary_agent.reached ==1:          
                self.env.primary_agent.accList.append(1)
            else:
                self.env.primary_agent.accList.append(0)
            with open("invalidRatio.txt", "a") as myfile:
